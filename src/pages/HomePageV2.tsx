  ...
  return (
    <div className="min-h-screen bg-white text-slate-950 overflow-x-hidden">
      ...
      <main className="pt-[76px]">
        ...
        <section className="py-16 md:py-20 bg-[#f8faff]"><div className="container mx-auto px-4"><div className="flex items-end justify-between gap-4 mb-8"><div><p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[#4324a5]">Property Intelligence</p><h2 className="mt-2 text-3xl md:text-4xl font-black text-[#151a3d]">Real Estate Insights</h2></div><Link to="/property-intelligence" className="text-sm font-bold text-[#4324a5]">View Insights</Link></div><div className="grid md:grid-cols-3 gap-6">{[
          [stockImages.villaPool, "Before You Shortlist a Property", "A practical way to compare property options before moving to the next step."],
          [stockImages.commercial, "Understanding Commercial Real Estate", "What businesses and investors should consider when evaluating commercial space."],
          [stockImages.land, "Plots, Land and Development Potential", "How location, access and surrounding growth influence land decisions."],
        ].map(([image, title, copy]) => <Link key={title} to="/property-intelligence" className="overflow-hidden rounded-2xl border border-[#e3e8f5] bg-white"><img src={image} alt={title} className="aspect-[16/9] w-full object-cover" /><div className="p-5"><h3 className="font-extrabold text-[#171c42]">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{copy}</p></div></Link>)}</div></div></section>
