...
export default async function handler(req,res){
  ...
  try{
    ...
    const rows=id
      ? await sql`SELECT public_id,property_type,location,area,area_unit,facing,total_valuation,per_unit_valuation,bedrooms,apartment_name,property_age,floors,constructed_area,parking,godown_plot_type,notes,photos,verified_at,published_at,updated_at FROM property_listings WHERE public_id=${id} AND verification_status='APPROVED' AND status='VERIFIED' AND publish_status='PUBLISHED' AND availability_status='AVAILABLE' LIMIT 1`
      : await sql`SELECT public_id,property_type,location,area,area_unit,facing,total_valuation,per_unit_valuation,bedrooms,apartment_name,property_age,floors,constructed_area,parking,godown_plot_type,notes,photos,verified_at,updated_at FROM property_listings WHERE verification_status='APPROVED' AND status='VERIFIED' AND publish_status='PUBLISHED' AND availability_status='AVAILABLE' ORDER BY verified_at DESC NULLS LAST,updated_at DESC LIMIT 250`;
